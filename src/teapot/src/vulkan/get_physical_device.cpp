...
void check_required_device_extensions(VkPhysicalDevice const physicalDevice, std::vector<char const *> const & requiredExtensions)
{
	...
	{
		if (std::find_if(std::begin(extensions), std::end(extensions), [element](VkExtensionProperties const & extensionProp) { return strcmp(element, extensionProp.extensionName) == 0; }) == std::end(extensions))
			throw std::runtime_error{"extension " + std::string{element} + " is not supported"};
	}
}

void check_device_suitability(VkPhysicalDevice const physicalDevice, std::vector<char const *> const & requiredExtensions)
{
...
std::tuple<uint32_t, uint32_t> get_device_graphics_and_present_queue_families(VkPhysicalDevice const physicalDevice, VkSurfaceKHR const surface)
{
